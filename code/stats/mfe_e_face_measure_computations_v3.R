...
# This version computes hit rates by binning the face delay. This script computes three bins (early vs. mid vs. late).
# Early is below 334 ms mid is between 334 and 667 ms and late is after 667 ms.
...
proc_fileName <- "processed_data_mfe_e_Proj_v3.csv" # output filename
...
main_df <- setNames(data.frame(matrix(ncol = 60, nrow = 0)), c("participant_id", "congAcc", "incongAcc",
                                                               ...
                                                               "num_early_incong_errorFaces_reported_new", "num_early_incong_errorFaces_reported_old", "mid_error_hitRate",
                                                               "mid_correct_hitRate", "num_mid_incong_correctFaces_reported_new", "num_mid_incong_correctFaces_reported_old",
                                                               "num_mid_incong_errorFaces_reported_new", "num_mid_incong_errorFaces_reported_old","late_error_hitRate",
                                                               ...
                                                               "early_incong_correct_miss_num", "early_hitRate_error_minus_correct", "mid_incong_error_hit_num",
                                                               "mid_incong_correct_hit_num", "mid_incong_error_miss_num",
                                                               "mid_incong_correct_miss_num", "mid_hitRate_error_minus_correct", "late_incong_error_hit_num",
    ...
    if (mean(as.numeric(flanker_df$current_trial_accuracy)) >= 0.6){
      ...
      if (nrow(legit_error_incong_flankerDat) >= 8 ){
        ...
        if (num_incong_error_faces_in_surp >= 8){
          ...
          ############################################## EARLY BIN #####################################################
          ##############################################################################################################
          early_bin_error_incong_flankerDat <- filter(error_incong_flankerDat, current_trial_face_delay_time < 0.334)
          early_bin_correct_incong_flankerDat <- filter(incong_corrDat, current_trial_face_delay_time < 0.334)
          ...
          ############################################## Middle BIN ####################################################
          ##############################################################################################################
          mid_bin_error_incong_flankerDat <- filter(error_incong_flankerDat, current_trial_face_delay_time >= 0.334 & current_trial_face_delay_time <=0.667 )
          mid_bin_correct_incong_flankerDat <- filter(incong_corrDat, current_trial_face_delay_time >= 0.334 & current_trial_face_delay_time <=0.667)

          ### number of incong error faces identified as old
          num_mid_incong_errorFaces_reported_old <- 0 # this is the number of incongruent error faces that they report as OLD and will be updated in the loop below:
          num_mid_incong_errorFaces_reported_new <- 0
          for (iii in 1:nrow(mid_bin_error_incong_flankerDat)){
            if (mid_bin_error_incong_flankerDat$current_trial_legitResponse[iii] == 1){ # if this trial is legit
              temp_face_from_flanker <- mid_bin_error_incong_flankerDat$current_trial_face[iii]
              temp_face_row_in_surp <- filter(surprise_df, face == temp_face_from_flanker)
              if (nrow(temp_face_row_in_surp) == 1){ # if old incong error face exist in the surprise_df
                if (temp_face_row_in_surp$surpAcc == 1){ # the old face is chosen as old correctly
                  num_mid_incong_errorFaces_reported_old <- num_mid_incong_errorFaces_reported_old + 1 # The number of incongruent error faces that they report as OLD
                } else if (temp_face_row_in_surp$surpAcc == 0){ # the new face is chosen as old incorrectly
                  num_mid_incong_errorFaces_reported_new <- num_mid_incong_errorFaces_reported_new + 1 # The number of incongruent error faces that they report as New
                }
              }
            }
          } # closing the loop over error incong flankerDat
          ...
          ### number of incong correct faces identified as old
          num_mid_incong_correctFaces_reported_old <- 0 # this is the number of incongruent correct faces that they report as Old and will be updated in the loop below:
          num_mid_incong_correctFaces_reported_new <- 0
          for (iii in 1:nrow(mid_bin_correct_incong_flankerDat)){
            if (mid_bin_correct_incong_flankerDat$current_trial_legitResponse[iii] == 1){ # if this trial is legit
              temp_face_from_flanker <- mid_bin_correct_incong_flankerDat$current_trial_face[iii]
              temp_face_row_in_surp <- filter(surprise_df, face == temp_face_from_flanker)
              if (nrow(temp_face_row_in_surp) == 1){ # if old incong correct face exist in the surprise_df
                if (temp_face_row_in_surp$surpAcc == 1){ # the old face is chosen as old correctly
                  num_mid_incong_correctFaces_reported_old <- num_mid_incong_correctFaces_reported_old + 1 # The number of incongruent correct faces that they report as Old
                } else if (temp_face_row_in_surp$surpAcc == 0){ # the new face is chosen as old incorrectly
                  num_mid_incong_correctFaces_reported_new <- num_mid_incong_correctFaces_reported_new + 1 # The number of incongruent correct faces that they report as New
                }
              }
            }
          } # closing the loop over correct incong flankerDat
          ...
          ############################################## LATE BIN #####################################################
          ##############################################################################################################
          late_bin_error_incong_flankerDat <- filter(error_incong_flankerDat, current_trial_face_delay_time > 0.667)
          late_bin_correct_incong_flankerDat <- filter(incong_corrDat, current_trial_face_delay_time > 0.667)
          ...
          if (nrow(mid_bin_error_incong_flankerDat) >= 8){
            mid_incong_error_hit_num <- num_mid_incong_errorFaces_reported_old
            mid_incong_error_miss_num <- num_mid_incong_errorFaces_reported_new
            mid_error_hitRate <- (mid_incong_error_hit_num) / ((mid_incong_error_hit_num) + mid_incong_error_miss_num) # hit rate
          } else if (nrow(mid_bin_error_incong_flankerDat) < 8){
            mid_incong_error_hit_num <- NA
            mid_incong_error_miss_num <- NA
            mid_error_hitRate <- NA
          }
          ...
          mid_incong_correct_hit_num <- num_mid_incong_correctFaces_reported_old
          mid_incong_correct_miss_num <- num_mid_incong_correctFaces_reported_new
          mid_correct_hitRate <- (mid_incong_correct_hit_num) / ((mid_incong_correct_hit_num) + mid_incong_correct_miss_num) # hit rate
          ...
          if (nrow(mid_bin_error_incong_flankerDat) >= 8){
            mid_hitRate_error_minus_correct <- mid_error_hitRate - mid_correct_hitRate
          } else if (nrow(mid_bin_error_incong_flankerDat) < 8){
            mid_hitRate_error_minus_correct <- NA
          }
